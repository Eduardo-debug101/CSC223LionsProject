...
public class Simulator {
        ...
        while (customersServedAndLeft != numCustomers) {
            ...
            if (!waitingCustomers.isEmpty()) {
                ...
                while (flag && !waitingCustomers.isEmpty()) {
                    ...
                    time, then it will add it to the selected queue and remove the customer
            ...
            // If someone is currently checking out then those who are still in self queue must wait longer
            if (selectedLane.isInUse() && !selfQueue.empty()) {
     ...
     * <p> Updates satisfiedCusts/dissatisfiedCusts globals if the Customer was happy or not
     * <p> Updates total queue time for stats
     *
    ...
    /**
     * Removes the Customer using the self service lane if they are ready to leave via finishTime
     * <p> Updates satisfiedCusts/dissatisfiedCusts globals if the Customer was happy or not
     * <p> Updates total lane time for stats
     *
     ...
     * <p>If the Customer's coinFlip is 1, they are placed in the smallest full service queue
     *
     ...
     * @return The best queue for a Customer based on the Customer's coinFlip as well as queue size
     */
    ...
    /**
     * This is for making sure all Customer's with the same arrival time have a chance to be added into a lane if there is an available one
     *
    ...
    /**
     * When being added to a queue, calculates wait time for the customer by getting the absolute value of the
     * leave time of the person in front of the queue minus the arrival time of the current customer.
     *
    ...
    private void output(LinkedQueue self, ArrayList<CheckoutLane> selfLanes, ArrayList<LinkedQueue> fullQueues, int time) {
